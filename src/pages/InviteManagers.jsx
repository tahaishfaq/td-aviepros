...
//           )
//         );
//         toast.error(res.data.message.message || "Failed to invite manager.");
//       }
//     } catch (error) {
...
const InviteManagers = () => {
  ...
  const fetchManagers = async () => {
    ...
    try {
      ...
      const formattedData = res?.data?.data?.map((item) => ({
        ...item,
        member_type: item.member_type === "Non-Resident" ? "Ground Workers" : item.member_type === "Secondary" ? "Managers" : item.member_type,
      })) || [];
  ...
  const inviteManager = async (managerId) => {
    ...
    try {
      ...
      // Handle the API response based on the structure shown in the screenshot
      if (res.data.message.status === "success") {
        console.log("Manager invited successfully:", res.data);
        toast.success("Manager invited successfully");
        // Ensure the managers state remains an array of objects
        setManagers((prevManagers) =>
          prevManagers.map((manager) =>
            manager.name === managerId
              ? { ...manager, access_status: "Invited" }
  ...
  const revokeManager = async (managerId) => {
    ...
    try {
      ...
      if (res.data.message.status === "success") {
        console.log("Manager access revoked successfully:", res.data);
              ...
              {isLoading ? (
                ...
                >
                  ...
                  >
                    ...
                    />
                    <path
                      d="M3.71429 19C2.76751 19 2 17.2857C2 14.9188 3.91878 13 6.28571 13"
                      stroke="currentColor"
                      stroke-width="1.5"
                      stroke-linecap="round"
                      stroke-linejoin="round"
